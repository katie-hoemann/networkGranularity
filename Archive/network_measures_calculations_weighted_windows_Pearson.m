...
for i_subject = 1:length(subjectIDlist)
    ...
    for i_day = 1:length(dayIDlist)
        ...
        dayMatrix = corrcoef(dayData); % calculate n-by-n correlation matrix
        % delete NaN values from correlation matrix
        i_corr = 1;
        skipDay = 0;
        missingCorr = find(isnan(dayMatrix(i_corr,:)));
        ...
        dayMatrix(missingCorr,:) = []; % delete missing rows
        dayMatrix(:,missingCorr) = []; % delete missing columns
        dayMatrix(logical(eye(size(dayMatrix)))) = 0; % set diagonal to 0 for BCT functions
        % transform into graph and prune edges using minimum spanning tree
        if spanningTree == 1
            G = graph(dayMatrix); % create graph object (use plot function to visualize)
...
%             numEdges(i_day) = numedges(Tpos); % record number of edges after pruning
            d = distances(Tpos); % get shortest paths between all nodes
%             diameter(i_day) = max(d(:)); % calculate diameter of the graph
            dayMatrix = full(adjacency(Tpos,'weighted')); % return the full storage version of the pruned graph
        end 
        ...
        dayM0(missingCorr) = []; % delete missing node from community affiliation vector
...
%         networkMeasures(:,16) = diameter;
%         networkMeasures(:,17) = numEdges;
%     end
    ...
    if spanningTree == 1
        networkMeasures = array2table(networkMeasures,'VariableNames',{'ppDay','cluster','clusterSD','density','numCom','mod','pPos','pNeg','pPosSD','pNegSD','dPos','dNeg','dPosSD','dNegSD','comRadius','modVA','diameter','numEdges'});
    else
...
%         if spanningTree == 1
%             diameter_Table_S = horzcat(window,windowSimpleMeans_Table(:,16));
%             diameter_Table_S.Properties.VariableNames = {'Window',ppID};
%             edges_Table_S = horzcat(window,windowSimpleMeans_Table(:,17));
%             edges_Table_S.Properties.VariableNames = {'Window',ppID};
%         end
...
%         if spanningTree == 1
%             diameter_Table_W = horzcat(window,windowWeightedMeans_Table(:,16));
%             diameter_Table_W.Properties.VariableNames = {'Window',ppID};
%             edges_Table_W = horzcat(window,windowWeightedMeans_Table(:,17));
%             edges_Table_W.Properties.VariableNames = {'Window',ppID};
%         end
    else
...
%         if spanningTree == 1
%             diameter_Table_S = horzcat(diameter_Table_S,windowSimpleMeans_Table(:,16));
%             diameter_Table_S.Properties.VariableNames{i_subject+1} = ppID;
%             edges_Table_S = horzcat(edges_Table_S,windowSimpleMeans_Table(:,17));
%             edges_Table_S.Properties.VariableNames{i_subject+1} = ppID;
%         end
...
%         if spanningTree == 1
%             diameter_Table_W = horzcat(diameter_Table_W,windowWeightedMeans_Table(:,16));
%             diameter_Table_W.Properties.VariableNames{i_subject+1} = ppID;
%             edges_Table_W = horzcat(edges_Table_W,windowWeightedMeans_Table(:,17));
%             edges_Table_W.Properties.VariableNames{i_subject+1} = ppID;
%         end
    end
...
if spanningTree == 1
    variableNames = {'cluster','clusterSD','density','numCom','mod','pPos','pNeg','pPosSD','pNegSD','dPos','dNeg','dPosSD','dNegSD','comRadius','modVA','diameter','edges'};
end
...
%     if spanningTree == 1
%         writetable(diameter_Table_S,['dataSet' num2str(dataSet) '_diameter_windows_simple.xlsx']);
%         writetable(edges_Table_S,['dataSet' num2str(dataSet) '_edges_windows_simple.xlsx']);
%     end
...
%     if spanningTree == 1
%         writetable(diameter_Table_W,['dataSet' num2str(dataSet) '_diameter_windows_weighted.xlsx']);
%         writetable(edges_Table_W,['dataSet' num2str(dataSet) '_edges_windows_weighted.xlsx']);
%     end
end
...
%     if spanningTree == 1
%         diameter_Array_S = table2array(diameter_Table_S)';
%         diameter_Array_S = horzcat(subjectCol,diameter_Array_S);
%         save(['dataSet' num2str(dataSet) '_diameter_windows_simple.mat'],'diameter_Array_S');
%         
%         edges_Array_S = table2array(edges_Table_S)';
%         edges_Array_S = horzcat(subjectCol,edges_Array_S);
%         save(['dataSet' num2str(dataSet) '_edges_windows_simple.mat'],'edges_Array_S');
%     end
...
%     if spanningTree == 1
%         diameter_Array_W = table2array(diameter_Table_W)';
%         diameter_Array_W = horzcat(subjectCol,diameter_Array_W);
%         save(['dataSet' num2str(dataSet) '_diameter_windows_weighted.mat'],'diameter_Array_W');
%         
%         edges_Array_W = table2array(edges_Table_W)';
%         edges_Array_W = horzcat(subjectCol,edges_Array_W);
%         save(['dataSet' num2str(dataSet) '_edges_windows_weighted.mat'],'edges_Array_W');
%     end
end
