% This script generates, for a given subject in a given data set, a
% histogram of raw connection weights (i.e., correlation values), the
% number of positive, negative, and null links, a heatmap
% of the correlation matrix, and sub-plotted histograms of raw intensity
% ratings for every emotion term
...
%% specify dataset and subject number, version parameters
...
if dataSet == 18
    ...
    for i_emotion = 1:length(remainingNodes)
        ...
        histogram(subjectData(:,i_emotion),edges);
        ylim([0 150]);
        xlim([0 7]);
...
positiveLinks = sum(halfSubjectMatrix > 0);
negativeLinks = sum(halfSubjectMatrix < 0);
nullLinks = (numel(subjectMatrix)-length(subjectMatrix))/2-(positiveLinks + negativeLinks);
