...
%% load data and word files 
%dataFile = '18ARIEOD_n35_100118.xlsx';
...
%% set parameters
startRatingsat1 = 0; % set to 1 if emotion intensity scale started at 1
maximumRating = 6; % set to highest valid scale value
...
%% remove subjects with bad data (88 term data set)
    ...
    for i_gamma = 1:nGamma
            ...
            Q0 = -1; Q1 = 0;    % initialize modularity values
                while Q1 - Q0 > 1e-5    % while modularity increases
                    ...
                    [M, Q1] = community_louvain(W,gArray(i_gamma),[],'negative_asym'); %UPDATED 10.10.18; previously 'sym'
                end
...
%diffComMax = diff(maxCommunity);
%elbowMax = max(diffComMax);
%gammaOptimalbyElbowMax = gArray(diffComMax==elbowMax)+gStepSize;
...
rawICCs = array2table(rawICC,'RowNames',cellstr(num2str(subjectIDlist)),'VariableNames',{'NegV','PosV','MeanV','NegV_LowA','PosV_HighA','NegV_HighA','PosV_LowA'});
granularity = array2table(gran,'RowNames',cellstr(num2str(subjectIDlist)),'VariableNames',{'NegV','PosV','MeanV','NegV_LowA','PosV_HighA','NegV_HighA','PosV_LowA'});
zICCs = array2table(rtoZ,'RowNames',cellstr(num2str(subjectIDlist)),'VariableNames',{'NegV','PosV','MeanV','NegV_LowA','PosV_HighA','NegV_HighA','PosV_LowA'});
zInvs = array2table(zInv,'RowNames',cellstr(num2str(subjectIDlist)),'VariableNames',{'NegV','PosV','MeanV','NegV_LowA','PosV_HighA','NegV_HighA','PosV_LowA'});
...
%% To do EOD 1 November 2018: 
% - drop PPs that are missing > 0.25 nodes (K) - DONE
% - run and visualize (comm assignment) for 18 and 88 term data sets (K)
% - create separate script for gamma tuning from other calculations (K)
% - clear datasets of incorrect/missing values (K) - DONE
% - outlier detection for non-normal distributions ("isoutlier'; Maria)
% - other community detection algorithms (Melody)

% - remove participants with less than certain amount of variance?
% - clean all lists/matrices of removed participants?
% - calculate additional network metrics? which ones? (Maria)
% - create optional script/code to output individual subject data
