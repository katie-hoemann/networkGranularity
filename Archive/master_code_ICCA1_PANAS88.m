...
for i_subject = 1:length(subjectIDList)
    ...
    %% Computation of ICC
    ICC_mat(i_subject,1) = ICC(score(:,(labels(:,1)==0)),'A-1');
    ICC_mat(i_subject,2) = ICC(score(:,(labels(:,1)==1)),'A-1');
    ICC_mat(i_subject,3) = ICC(score(:,(labels(:,2)==0)),'A-1');
    ICC_mat(i_subject,4) = ICC(score(:,(labels(:,2)==1)),'A-1');
end

ReportTable = array2table(ICC_mat,'RowNames',cellstr(num2str(subjectIDList)),'VariableNames',{'Neg_V',...
    'Pos_V','Low_A','High_A'});
writetable(ReportTable,'ICC_A1.dat','WriteRowNames',true);
writetable(ReportTable,'ICC_A1.txt','WriteRowNames',true);
