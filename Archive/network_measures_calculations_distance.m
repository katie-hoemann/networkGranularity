...
for i_subject = 1:length(subjectIDlist)
    ...
    for i_var = 1:length(rows)
        for j_var = rows(rows~=i_var)
            subjectMatrix(i_var,j_var) = distcorr(subjectData(:,i_var),subjectData(:,j_var));
        end
    end
    ...
    subjectM0(missingCorr) = []; % delete missing nodes from community affiliation vector
    ...
    clear subjectMatrix missingCorr
end
