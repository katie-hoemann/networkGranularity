...
%datafile = 'ES39data.xlsx';
datafile = 'ARIEOD_n35_100118.xlsx';
...
for i_subject = 1:length(subjectIDlist)
    ...
    subjectMatrix(missing_corr,:) = []; % delete missing row
    subjectMatrix(:,missing_corr) = []; % delete missing column
...
ICCtoModularity = corr(rtoZ_mat(:,3),modularity);
ICCtoCommunity = corr(rtoZ_mat(:,3),community);
...
%to do EOD 11 April 2018: 
% - why is it almost always the last fold that does the best?
% - run it without averages the gamma values to test
% - ensure adaptable for 39-term dataset (work out deletion of sparse datasets)
% - run clustering coefficient script for node-level and global measures
% [C,C_null,Ctot,Ctot_null]=clustering_coef_wu_sign(W,3);
% clusterCoef(i_subject)=Ctot; to save off global value
% C is a vector per node, so save that off for node level measure

% Add this to the script but comment it out (alongside 0-ing out diagonal):
...
% - run correlations against RDEES, TAS, and any other measures
% - 39 term data located under studies > experience sampling > analysis >
% master data newer file
