...
for i_subject = 1:length(subjectIDList)
    ...
    %% Rescale to from 1-5 to 0-4
    data = data-1;
    %% Computation of ICCs - by hemispheres and quadrants
    rawICC_mat(i_subject,1) = ICC(data(:,(labels(:,1)==0)),'A-1'); %edit 'score' to 'data'
    rawICC_mat(i_subject,2) = ICC(data(:,(labels(:,1)==1)),'A-1'); %edit 'score' to 'data'
    rawICC_mat(i_subject,3) = ICC(data(:,(labels(:,2)==0)),'A-1'); %edit 'score' to 'data'
    rawICC_mat(i_subject,4) = ICC(data(:,(labels(:,2)==1)),'A-1'); %edit 'score' to 'data'
    rawICC_mat(i_subject,5) = ICC(data(:,(labels(:,1)==0 & labels(:,2)==0)),'A-1');
    rawICC_mat(i_subject,6) = ICC(data(:,(labels(:,1)==1 & labels(:,2)==1)),'A-1');
    rawICC_mat(i_subject,7) = ICC(data(:,(labels(:,1)==0 & labels(:,2)==1)),'A-1');
    rawICC_mat(i_subject,8) = ICC(data(:,(labels(:,1)==1 & labels(:,2)==0)),'A-1');
...
writetable(ReportTableICC,'ICC_A1.dat','WriteRowNames',true);
writetable(ReportTableICC,'ICC_A1.txt','WriteRowNames',true);

ReportTableGran = array2table(gran_mat,'RowNames',cellstr(num2str(subjectIDList)),'VariableNames',{'Neg_V',...
    'Pos_V','Low_A','High_A','NegLow','PosHigh','NegHigh','PosLow'});
writetable(ReportTableGran,'Gran_A1.dat','WriteRowNames',true);
writetable(ReportTableGran,'Gran_A1.txt','WriteRowNames',true);

ReportTableRtoZ = array2table(rtoz_mat,'RowNames',cellstr(num2str(subjectIDList)),'VariableNames',{'Neg_V',...
    'Pos_V','Low_A','High_A','NegLow','PosHigh','NegHigh','PosLow'});
writetable(ReportTableRtoZ,'RtoZ_A1.dat','WriteRowNames',true);
writetable(ReportTableRtoZ,'RtoZ_A1.txt','WriteRowNames',true);

ReportTableZInv = array2table(zinv_mat,'RowNames',cellstr(num2str(subjectIDList)),'VariableNames',{'Neg_V',...
    'Pos_V','Low_A','High_A','NegLow','PosHigh','NegHigh','PosLow'});
writetable(ReportTableZInv,'ZInv_A1.dat','WriteRowNames',true);
writetable(ReportTableZInv,'ZInv_A1.txt','WriteRowNames',true);
