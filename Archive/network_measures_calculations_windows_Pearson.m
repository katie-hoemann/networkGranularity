...
for i_subject = 1:length(subjectIDlist)
    ...
    for i_window = 1:length(dayIDlist)-2
        ...
        numInstancesWindow(i_window) = size(windowData,1);
        ...
        numNodesWindow(i_window) = size(windowMatrix,1);
        ...
        if spanningTree == 1
            G = graph(windowMatrix); % create graph object (use plot function to visualize)
            ...
            numEdges(i_window) = numedges(Tpos); % record number of edges after pruning
            d = distances(Tpos); % get shortest paths between all nodes
            diameter(i_window) = max(d(:)); % calculate diameter of the graph
            windowMatrix = full(adjacency(Tpos,'weighted')); % return the full storage version of the pruned graph
        end 
        ...
        windowM0(missingCorr) = []; % delete missing node from community affiliation vector
    ...
    numNodes(i_subject) = mean(numNodesWindow);
    numInstances(i_subject) = mean(numInstancesWindow); 
    ...
    if spanningTree == 1
        ...
        networkMeasureMeans(i_subject,16) = mean(diameter,'omitnan');
        networkMeasureSDs(i_subject,16) = std(diameter,'omitnan');
        networkMeasureMeans(i_subject,17) = mean(numEdges,'omitnan');
        networkMeasureSDs(i_subject,17) = std(numEdges,'omitnan');
    end
    ...
    if i_subject == 1
        ...
        if spanningTree == 1
            diameter_Table = tablecompile_start(window,windowNumbers,diameter',ppID,'Window','ppWindow','ppWindow');
            edges_Table = tablecompile_start(window,windowNumbers,numEdges',ppID,'Window','ppWindow','ppWindow');
        end
    else
        ...
        if spanningTree == 1
            diameter_Table = tablecompile_iter(diameter_Table,windowNumbers,diameter',ppID,'Window','ppWindow','ppWindow');
            edges_Table = tablecompile_iter(edges_Table,windowNumbers,numEdges',ppID,'Window','ppWindow','ppWindow');
        end
    end
    clear numInstancesWindow numNodesWindow clusterCoefGlobal clusterCoefGlobalSD kdenW numCommunities modularity participation participationSD diversity diversitySD commRadius modularity_VA diameter numEdges
end
...
if printTables == 1
    ...
    if spanningTree == 1
        writetable(diameter_Table,['dataSet' num2str(dataSet) '_diameter_windows.xlsx']);
        writetable(edges_Table,['dataSet' num2str(dataSet) '_edges_windows.xlsx']);
    end
end
...
if saveData == 1
    ...
    if spanningTree == 1
        diameter_Array = table2array(diameter_Table)';
        diameter_Array = horzcat(subjectCol,diameter_Array);
        save(['dataSet' num2str(dataSet) '_diameter_windows.mat'],'diameter_Array');
        
        edges_Array = table2array(edges_Table)';
        edges_Array = horzcat(subjectCol,edges_Array);
        save(['dataSet' num2str(dataSet) '_edges_windows.mat'],'edges_Array');
    end
end
...
mean(numInstances)
std(numInstances)
sampleData = table2array(numCom_Table(:,2:end));
for i_subject = 1:length(subjectIDlist)
    subjectColumn = sampleData(:,i_subject);
    subjectColumn = subjectColumn(~isnan(subjectColumn));
    numWindows(i_subject) = numel(subjectColumn);
end
mean(numWindows)
std(numWindows)